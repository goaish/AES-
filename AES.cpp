#include <iostream>
#include <iomanip>
#include <sstream>
#include <string>
#include <cstdint>
using namespace std;

const uint8_t SBOX[256] = {
    
};

const uint8_t INV_SBOX[256] = {
    
};

const uint8_t RCON[11] = {0x00,0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1b,0x36};

uint8_t gf_mul(uint8_t a, uint8_t b) {
    uint8_t p = 0;
    for (int i = 0; i < 8; ++i) {
        if (b & 1) p ^= a;
        bool hi = (a & 0x80);
        a <<= 1;
        if (hi) a ^= 0x1b;
        b >>= 1;
    }
    return p;
}

void sub_bytes(uint8_t state[4][4]) {
    for (int r = 0; r < 4; ++r)
        for (int c = 0; c < 4; ++c)
            state[r][c] = SBOX[state[r][c]];
}

void inv_sub_bytes(uint8_t state[4][4]) {
    for (int r = 0; r < 4; ++r)
        for (int c = 0; c < 4; ++c)
            state[r][c] = INV_SBOX[state[r][c]];
}

void shift_rows(uint8_t state[4][4]) {
    // row 1 left rotate by 1
    uint8_t tmp = state[1][0];
    state[1][0] = state[1][1];
    state[1][1] = state[1][2];
    state[1][2] = state[1][3];
    state[1][3] = tmp;
    // row 2 left rotate by 2
    swap(state[2][0], state[2][2]);
    swap(state[2][1], state[2][3]);
    // row 3 left rotate by 3 (or right by 1)
    tmp = state[3][3];
    state[3][3] = state[3][2];
    state[3][2] = state[3][1];
    state[3][1] = state[3][0];
    state[3][0] = tmp;
}

void inv_shift_rows(uint8_t state[4][4]) {
    // row1 right rotate by 1
    uint8_t tmp = state[1][3];
    state[1][3] = state[1][2];
    state[1][2] = state[1][1];
    state[1][1] = state[1][0];
    state[1][0] = tmp;
    // row2 right rotate by 2 (same as left rotate by 2)
    swap(state[2][0], state[2][2]);
    swap(state[2][1], state[2][3]);
    // row3 right rotate by 3 (or left by 1)
    tmp = state[3][0];
    state[3][0] = state[3][1];
    state[3][1] = state[3][2];
    state[3][2] = state[3][3];
    state[3][3] = tmp;
}

void mix_columns(uint8_t state[4][4]) {
    for (int c = 0; c < 4; ++c) {
        uint8_t s0 = state[0][c], s1 = state[1][c], s2 = state[2][c], s3 = state[3][c];
        state[0][c] = gf_mul(s0, 2) ^ gf_mul(s1, 3) ^ s2 ^ s3;
        state[1][c] = s0 ^ gf_mul(s1, 2) ^ gf_mul(s2, 3) ^ s3;
        state[2][c] = s0 ^ s1 ^ gf_mul(s2, 2) ^ gf_mul(s3, 3);
        state[3][c] = gf_mul(s0, 3) ^ s1 ^ s2 ^ gf_mul(s3, 2);
    }
}

void inv_mix_columns(uint8_t state[4][4]) {
    for (int c = 0; c < 4; ++c) {
        uint8_t s0 = state[0][c], s1 = state[1][c], s2 = state[2][c], s3 = state[3][c];
        state[0][c] = gf_mul(s0, 0x0e) ^ gf_mul(s1, 0x0b) ^ gf_mul(s2, 0x0d) ^ gf_mul(s3, 0x09);
        state[1][c] = gf_mul(s0, 0x09) ^ gf_mul(s1, 0x0e) ^ gf_mul(s2, 0x0b) ^ gf_mul(s3, 0x0d);
        state[2][c] = gf_mul(s0, 0x0d) ^ gf_mul(s1, 0x09) ^ gf_mul(s2, 0x0e) ^ gf_mul(s3, 0x0b);
        state[3][c] = gf_mul(s0, 0x0b) ^ gf_mul(s1, 0x0d) ^ gf_mul(s2, 0x09) ^ gf_mul(s3, 0x0e);
    }
}

void key_expansion(const uint8_t* key, uint8_t* expanded_key) {
    for (int i = 0; i < 16; ++i) expanded_key[i] = key[i];
    int bytes_generated = 16;
    int rcon_iter = 1;
    uint8_t temp[4];

    while (bytes_generated < 176) {
        for (int i = 0; i < 4; ++i) temp[i] = expanded_key[bytes_generated - 4 + i];
        if (bytes_generated % 16 == 0) {
            uint8_t t = temp[0];
            temp[0] = temp[1];
            temp[1] = temp[2];
            temp[2] = temp[3];
            temp[3] = t;
            temp[0] = SBOX[temp[0]] ^ RCON[rcon_iter++];
            temp[1] = SBOX[temp[1]];
            temp[2] = SBOX[temp[2]];
            temp[3] = SBOX[temp[3]];
        }
        for (int i = 0; i < 4; ++i) {
            expanded_key[bytes_generated] = expanded_key[bytes_generated - 16] ^ temp[i];
            ++bytes_generated;
        }
    }
}

void add_round_key(uint8_t state[4][4], const uint8_t* round_key) {
    // round_key is 16 bytes; mapping column-major: round_key[col*4 + row]
    for (int c = 0; c < 4; ++c)
        for (int r = 0; r < 4; ++r)
            state[r][c] ^= round_key[c*4 + r];
}

string pad(const string& input) {
    int pad_len = 16 - (input.length() % 16);
    return input + string(pad_len, static_cast<char>(pad_len));
}

string unpad(const string& input) {
    if (input.empty()) return {};
    int pad_len = static_cast<unsigned char>(input.back());
    if (pad_len <= 0 || pad_len > 16) return input; // simple safety
    return input.substr(0, input.length() - pad_len);
}

string bytes_to_hex(const string& data) {
    stringstream ss;
    ss << hex << setfill('0');
    for (unsigned char c : data) ss << setw(2) << static_cast<int>(c);
    return ss.str();
}

string encrypt(const string& plaintext, const uint8_t* key) {
    string padded = pad(plaintext);
    uint8_t expanded_key[176];
    key_expansion(key, expanded_key);
    uint8_t state[4][4];

    for (size_t block = 0; block < padded.size(); block += 16) {
        // load block into state (column-major)
        for (int c = 0; c < 4; ++c)
            for (int r = 0; r < 4; ++r)
                state[r][c] = static_cast<uint8_t>(padded[block + c*4 + r]);

        add_round_key(state, expanded_key); // round 0

        for (int round = 1; round <= 9; ++round) {
            sub_bytes(state);
            shift_rows(state);
            mix_columns(state);
            add_round_key(state, expanded_key + round * 16);
        }

        // final round (no mix)
        sub_bytes(state);
        shift_rows(state);
        add_round_key(state, expanded_key + 160);

        // write state back to padded
        for (int c = 0; c < 4; ++c)
            for (int r = 0; r < 4; ++r)
                padded[block + c*4 + r] = static_cast<char>(state[r][c]);
    }

    return padded;
}

string decrypt(const string& ciphertext, const uint8_t* key) {
    if (ciphertext.empty()) return {};
    uint8_t expanded_key[176];
    key_expansion(key, expanded_key);
    string out = ciphertext;
    uint8_t state[4][4];

    for (size_t block = 0; block < out.size(); block += 16) {
        for (int c = 0; c < 4; ++c)
            for (int r = 0; r < 4; ++r)
                state[r][c] = static_cast<uint8_t>(out[block + c*4 + r]);

        add_round_key(state, expanded_key + 160); // last round key

        // inverse rounds 9..1
        for (int round = 9; round >= 1; --round) {
            inv_shift_rows(state);
            inv_sub_bytes(state);
            add_round_key(state, expanded_key + round * 16);
            inv_mix_columns(state);
        }

        // final inverse round (round 0)
        inv_shift_rows(state);
        inv_sub_bytes(state);
        add_round_key(state, expanded_key);

        // write back
        for (int c = 0; c < 4; ++c)
            for (int r = 0; r < 4; ++r)
                out[block + c*4 + r] = static_cast<char>(state[r][c]);
    }

    return unpad(out);
}

int main() {
    uint8_t key[16] = {
        0x00,0x01,0x02,0x03,0x04,0x05,0x06,0x07,
        0x08,0x09,0x0a,0x0b,0x0c,0x0d,0x0e,0x0f
    };

    string plain = "Hello AES Implementation Test!";
    string cipher = encrypt(plain, key);
    string dec = decrypt(cipher, key);

    cout << "Plain:  " << plain << endl;
    cout << "Cipher: " << bytes_to_hex(cipher) << endl;
    cout << "Dec:    " << dec << endl;
    cout << "Match:  " << (plain == dec ? "YES" : "NO") << endl;

    return 0;
}
